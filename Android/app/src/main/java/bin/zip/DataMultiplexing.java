...
import com.mcal.apkdatamultiplexing.App;
import com.mcal.apkdatamultiplexing.utils.FileHelper;
...
public class DataMultiplexing {
    ...
    public void startMultiplexing(File input, File output, String hostEntryName) throws Exception {
        optimize(input, output, hostEntryName, true);
        final Context context = App.getContext();
        final String keyName = "test.jks";
        final File keyFile = new File(context.getFilesDir(), keyName);
        FileHelper.INSTANCE.copyFile(context.getAssets().open(keyName), new FileOutputStream(keyFile));
        V2V3SchemeSigner.sign(output, new JksSignatureKey(keyFile, "123456", "123456", "123456"), true, true);
        System.out.println("Check " + isZipFileContentEquals(input, output));
    }
